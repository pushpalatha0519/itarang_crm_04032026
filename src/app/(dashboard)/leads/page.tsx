...
import { PlusCircle, Search, Filter, Loader2, Phone } from 'lucide-react';
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <Link href="/leads/new" className="inline-flex items-center gap-2 px-4 py-2 bg-brand-600 text-white font-medium rounded-lg hover:bg-brand-700 transition-colors shadow-sm">
            ...
            {isCeo && (
                <AIDialerControls 
                    leads={leads.map((l: any) => ({ 
                        ...
                        manual_takeover: l.manual_takeover, 
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-200 text-xs uppercase text-gray-500 font-semibold tracking-wider">
                                    ...
                                    {isCeo && <th className="px-6 py-4">AI Status</th>}
                                    ...
                                    <tr key={lead.id} className={`hover:bg-gray-50 transition-colors group ${newLeadId === lead.id ? 'bg-brand-50' : ''}`}>
                                        ...
                                        <td className="px-6 py-4 text-right space-x-2">
                                            <Link href={`/leads/${lead.id}/edit`}>
                                                <button className="text-indigo-600 hover:text-indigo-800 font-medium text-xs transition-opacity">
                                                    Edit
                                                </button>
                                            </Link>
                                            <Link href={`/leads/${lead.id}`}>
                                                <button className="text-brand-600 hover:text-brand-800 font-medium text-xs transition-opacity">
                                                    View
                                                </button>
                                            </Link>
                                        </td>
